...
/mob/living/carbon/Xenomorph/handle_vehicle_bump(var/obj/vehicle/multitile/V)
	...
	if(!mob_knocked_down)
		if(V.vehicle_flags & VEHICLE_CLASS_WEAK || V.vehicle_flags & VEHICLE_CLASS_LIGHT)
			...
			mob_moved = step(src, V.last_move_dir)
	...
	if(mob_knocked_down)
		return TRUE
	else if (mob_moved && last_move_dir & V.last_move_dir)
		if(momentum_penalty)
			V.move_momentum = Floor(V.move_momentum*0.7)
			V.update_next_move()
		return TRUE
	return FALSE
...
/mob/living/carbon/Xenomorph/Defender/handle_vehicle_bump(var/obj/vehicle/multitile/V)
	if(fortify)
		if(V.vehicle_flags & VEHICLE_CLASS_WEAK)	//defenders being able to completely block armored vehicles by crawling into a boulder is ridiculous
			...
			var/mob_moved = step(src, V.last_move_dir)
			V.move_momentum = Floor(V.move_momentum/2)
			V.update_next_move()
			return mob_moved
